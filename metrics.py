            ...
            for method in os.listdir(test_dir):
                ...
                if len(renders) > 0:
                    ...
                    if source_path is not None and "imed" in source_path.lower():
                        ...
                        # Save debug visualisations so you can inspect the evaluation masks.
                        debug_tool_dir = method_dir / "debug_tool_masks"
                        debug_eval_dir = method_dir / "debug_eval_masks"
                        debug_comp_dir = method_dir / "debug_composite"
                        for d in [debug_tool_dir, debug_eval_dir, debug_comp_dir]:
                        ...
                        for i, fname in enumerate(image_names):
                            ...
                            Image.fromarray(eval_np).save(debug_eval_dir / fname)
                            # composite: render | GT | eval_mask (side-by-side)
                            render_np = (renders[i].squeeze().permute(1, 2, 0).cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
                            gt_np = (gts[i].squeeze().permute(1, 2, 0).cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
                            eval_rgb = np.stack([eval_np, eval_np, eval_np], axis=-1)
                            composite = np.concatenate([render_np, gt_np, eval_rgb], axis=1)
                            Image.fromarray(composite).save(debug_comp_dir / fname)

                ssims = []
                psnrs = []
                lpipss = []
                                
                debug_masked_dir = method_dir / "debug_masked_renders"
                debug_masked_dir.mkdir(exist_ok=True)
                for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                    ...
                    # Save masked render (pixels outside eval mask are blacked out)
                    masked_render_np = ((render * mask).squeeze().permute(1, 2, 0).cpu().numpy() * 255).clip(0, 255).astype(np.uint8)
                    Image.fromarray(masked_render_np).save(debug_masked_dir / image_names[idx])
